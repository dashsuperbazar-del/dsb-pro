...
export async function replaceItemImage(input:{tenantId:string;itemId:string;blob:Blob;oldPath?:string|null}):Promise<string> {
 ...
 if(updated.error){ await client.storage.from('item-images').remove([path]); throw new Error(errorMessage(updated.error)); }
 if(input.oldPath && input.oldPath!==path){ const removed=await client.storage.from('item-images').remove([input.oldPath]); if(removed.error) console.warn('Old item image purge failed:',errorMessage(removed.error)); }
 return path;
}
