...
export async function checkInvariants(){const {data,error}=await getSupabaseClient().rpc('check_invariants');if(error)fail(error);return data as {ok:boolean;saleTotalViolations:number;purchaseTotalViolations:number;negativeStock:number;allocationViolations:number};}
