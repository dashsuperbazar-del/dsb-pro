...
usage() {
  echo "usage: $0 <database-url> <foundation|hardening|phase65|all> [...]" >&2
  exit 2
}
...
for group in "$@"; do
  case "$group" in
    foundation|hardening|phase65) requested["$group"]=1 ;;
    all) requested[foundation]=1; requested[hardening]=1; requested[phase65]=1 ;;
