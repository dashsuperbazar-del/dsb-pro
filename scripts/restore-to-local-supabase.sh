...
# Current GoTrue is single-instance: every user lookup is scoped to uuid.Nil,
# and auth.instances multi-instance rows are intentionally ignored. Hosted
# dumps carry a project-specific instance_id, so normalize recovered users to
# the nil UUID required by the fresh target's Auth service.
psql "$TARGET_DB_URL" -v ON_ERROR_STOP=1 <<'SQL'
update auth.users
