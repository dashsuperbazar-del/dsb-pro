...
export function CustomersScreen(){
  ...
  return <main class="page wide"><p><a href={appRoute.home}>← Home</a></p><h1>Customers & ledger</h1>
    ...
    {selected&&<section class="card"><h2>Receive payment / allocate invoices</h2><form onSubmit={receive}>
      ...
      <p>Allocated {money(allocated)} · Unallocated advance {money(Math.max(0,toPaise(amount)-allocated))}</p><button class="primary" disabled={busy||!shopId||!businessDate}>{busy?'Saving…':'Record payment'}</button>
    </form></section>}
