...
export function InventoryScreen() {
  ...
  async function addItem(ev:Event){ ev.preventDefault(); setError(''); const f=new FormData(ev.currentTarget as HTMLFormElement);
    try { const item=await createItem({tenantId,name:String(f.get('name')),sku:String(f.get('sku')||'')||undefined,unit1:String(f.get('unit1')),unit2:String(f.get('unit2')||'')||undefined,unit3:String(f.get('unit3')||'')||undefined,conv1:f.get('conv1')?Number(f.get('conv1')):undefined,conv2:f.get('conv2')?Number(f.get('conv2')):undefined,clientId:crypto.randomUUID()}); setMessage(`Created ${item.name}.`); (ev.currentTarget as HTMLFormElement).reset(); await refresh(); } catch(e){setError(String(e));}
  }
