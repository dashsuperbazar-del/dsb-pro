...
export function InventoryScreen() {
  ...
  async function addItem(ev:Event){ ev.preventDefault(); setError(''); const f=new FormData(ev.currentTarget as HTMLFormElement);
    try { const item=await createItem({tenantId,name:String(f.get('name')),sku:String(f.get('sku')||'')||undefined,unit1:String(f.get('unit1')),unit2:String(f.get('unit2')||'')||undefined,unit3:String(f.get('unit3')||'')||undefined,conv1:f.get('conv1')?Number(f.get('conv1')):undefined,conv2:f.get('conv2')?Number(f.get('conv2')):undefined,clientId:crypto.randomUUID()}); (ev.currentTarget as HTMLFormElement).reset(); await refresh(); setMessage(`Created ${item.name}.`); } catch(e){setError(String(e));}
  }
  ...
  async function addParty(ev:Event){ ev.preventDefault(); setError(''); const f=new FormData(ev.currentTarget as HTMLFormElement);
    try { const party=await createParty({tenantId,name:String(f.get('name')),phone:String(f.get('phone')||'')||undefined,gstin:String(f.get('gstin')||'')||undefined,clientId:crypto.randomUUID()}); (ev.currentTarget as HTMLFormElement).reset(); await refresh(); setMessage(`Created supplier ${party.name}.`); } catch(e){setError(String(e));}
  }
  ...
  return <main>
    ...
    <section><h2>Item master</h2><p>Unit order follows DSB: big → small → piece. Stock is stored in the smallest configured unit.</p><form onSubmit={addItem}><input name="name" placeholder="Item name" required/> <input name="sku" placeholder="SKU"/> <input name="unit1" placeholder="Big unit (e.g. case)" required/> <input name="unit2" placeholder="Small unit (e.g. pack)"/> <input name="conv1" type="number" min="0" step="any" placeholder="Small units per big"/> <input name="unit3" placeholder="Piece unit (optional)"/> <input name="conv2" type="number" min="0" step="any" placeholder="Pieces per small"/> <button>Create item</button></form>
      ...
      {selectedItem&&<form onSubmit={changePrice}><select name="kind"><option value="retail">Retail</option><option value="wholesale">Wholesale</option><option value="mrp">MRP</option><option value="cost_last">Last cost</option></select> <select name="unitLevel"><option value="1">{selectedItem.unit1}</option>{selectedItem.unit2&&<option value="2">{selectedItem.unit2}</option>}{selectedItem.unit3&&<option value="3">{selectedItem.unit3}</option>}</select> <input name="price" type="number" min="0" step="0.01" placeholder="₹ price" required/> <button>Set price</button></form>}
    </section>
    ...
    <section><h2>Post purchase</h2><form onSubmit={addPurchase}><input name="billNo" placeholder="Bill no."/> <input name="date" type="date" value={businessDate} onInput={e=>setBusinessDate((e.currentTarget as HTMLInputElement).value)} required/> <select name="partyId"><option value="">No supplier</option>{parties.map(p=><option value={p.id}>{p.name}</option>)}</select> <select name="itemId" required value={purchaseItemId} onChange={e=>setPurchaseItemId((e.currentTarget as HTMLSelectElement).value)}><option value="">Item…</option>{items.map(i=><option value={i.id}>{i.name}</option>)}</select> <select name="unitLevel" required disabled={!purchaseItem}><option value="1">{purchaseItem?.unit1||'Big unit'}</option>{purchaseItem?.unit2&&<option value="2">{purchaseItem.unit2}</option>}{purchaseItem?.unit3&&<option value="3">{purchaseItem.unit3}</option>}</select> <input name="qty" type="number" min="0.000001" step="any" placeholder="Qty" required/> <input name="price" type="number" min="0" step="0.01" placeholder="Unit cost ₹" required/> <input name="discount" type="number" min="0" step="0.01" placeholder="Discount ₹"/> <input name="extra" type="number" min="0" step="0.01" placeholder="Extra ₹"/> <button>Post purchase</button></form></section>
  </main>;
}
