-- A production-size catalog previously required two large sequential pulls:
-- items, barcodes and stock were capped at 5,000 while the Phase 6 gate uses
-- 10,000 of each. Returning the complete 10k catalog in the first compressed
-- response removes the second-response latency without weakening the gate.
...
begin
  ...
  from (
    ...
    order by i.updated_at,i.id limit 10000
  ) q;
  ...
  from (
    ...
    order by b.updated_at,b.id limit 10000
  ) q;
  ...
  from (
    ...
    order by c.updated_at,c.id limit 10000
  ) q;
  ...
  from (
    ...
    order by s.updated_at,s.item_id limit 10000
  ) q;
