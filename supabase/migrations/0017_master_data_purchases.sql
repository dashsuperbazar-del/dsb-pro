-- Phase 3: master data + purchases. Financial posting is RPC-only; stock is derived
-- from the append-only stock_movements ledger. Direct DELETE/stock INSERT grants
-- are intentionally absent.
create extension if not exists btree_gist with schema extensions;

insert into permissions(code, description) values
  ('MANAGE_MASTER_DATA','Create and update item/category/party master data'),
  ('POST_PURCHASES','Post purchase bills and resulting stock movements')
...
create table categories (
  ...
  name text not null, created_by uuid not null default auth.uid(), created_at timestamptz not null default now(),
...
create table items (
  ...
  unit1 text not null, unit2 text, unit3 text, conv1 numeric(18,6), conv2 numeric(18,6),
  tax_rate_bp integer not null default 0 check (tax_rate_bp between 0 and 10000),
  min_stock numeric(18,6) not null default 0 check (min_stock >= 0), image_path text,
...
create table item_barcodes (
  id uuid primary key default gen_random_uuid(), tenant_id uuid not null references tenants(id),
  item_id uuid not null references items(id), barcode text not null, unit_level smallint not null check(unit_level between 1 and 3),
  created_by uuid not null default auth.uid(), created_at timestamptz not null default now(),
  updated_at bigint not null default 0, deleted_at bigint, client_id text,
...
create table item_prices (
  ...
  unit_level smallint not null check(unit_level between 1 and 3), price_paise bigint not null check(price_paise >= 0),
  effective_from timestamptz not null default now(), effective_to timestamptz,
  created_by uuid not null default auth.uid(), created_at timestamptz not null default now(),
  updated_at bigint not null default 0, deleted_at bigint, client_id text,
  check(effective_to is null or effective_to > effective_from), unique(tenant_id,client_id)
);
alter table item_prices add constraint item_prices_no_overlap exclude using gist
  (tenant_id with =, item_id with =, (coalesce(shop_id,'00000000-0000-0000-0000-000000000000'::uuid)) with =,
   kind with =, unit_level with =,
   tstzrange(effective_from,coalesce(effective_to,'infinity'::timestamptz),'[)') with &&)
  where (deleted_at is null);

create table purchase_bills (
  id uuid primary key default gen_random_uuid(), tenant_id uuid not null references tenants(id),
  shop_id uuid not null references shops(id), party_id uuid references parties(id), bill_no text,
  business_date date not null default current_date, status text not null default 'POSTED' check(status in ('POSTED','VOID')),
  subtotal_paise bigint not null check(subtotal_paise >= 0), discount_paise bigint not null default 0 check(discount_paise >= 0),
  extra_charges_paise bigint not null default 0 check(extra_charges_paise >= 0), total_paise bigint not null check(total_paise >= 0),
  bill_image_path text, voided_at timestamptz,
  created_by uuid not null default auth.uid(), created_at timestamptz not null default now(),
...
create table purchase_bill_items (
  id uuid primary key default gen_random_uuid(), tenant_id uuid not null references tenants(id),
  shop_id uuid not null references shops(id), purchase_bill_id uuid not null references purchase_bills(id), item_id uuid not null references items(id),
  item_name_at_purchase text not null, unit_at_purchase text not null, conv1_at_purchase numeric(18,6), conv2_at_purchase numeric(18,6),
  unit_level smallint not null check(unit_level between 1 and 3), qty numeric(18,6) not null check(qty > 0),
  base_qty numeric(18,6) not null check(base_qty > 0), unit_price_paise bigint not null check(unit_price_paise >= 0),
  line_total_paise bigint not null check(line_total_paise >= 0), tax_rate_bp_at_purchase integer not null default 0,
  hsn_at_purchase text, created_by uuid not null default auth.uid(), created_at timestamptz not null default now(),
  updated_at bigint not null default 0, deleted_at bigint, client_id text,
  unique(tenant_id,client_id)
);
create table stock_movements (
  id uuid primary key default gen_random_uuid(), tenant_id uuid not null references tenants(id),
  shop_id uuid not null references shops(id), item_id uuid not null references items(id),
...
select tenant_id, shop_id, item_id, coalesce(sum(qty_base),0)::numeric(18,6) as qty_base
...
create index item_barcodes_item_idx on item_barcodes(tenant_id,item_id);
create index item_prices_lookup_idx on item_prices(tenant_id,item_id,kind,unit_level,effective_from desc);
create index purchase_bills_tenant_date_idx on purchase_bills(tenant_id,business_date desc);
...
create trigger audit_purchase_bill_items after insert or update on purchase_bill_items for each row execute function audit_row_change();
create trigger audit_stock_movements after insert on stock_movements for each row execute function audit_row_change();

alter table categories enable row level security; alter table parties enable row level security; alter table items enable row level security;
alter table item_barcodes enable row level security; alter table item_prices enable row level security; alter table purchase_bills enable row level security;
alter table purchase_bill_items enable row level security; alter table stock_movements enable row level security;
...
-- Financial rows and stock movements have no direct mutation grants.

create policy categories_read on categories for select using(tenant_id=current_tenant_id());
...
create policy parties_read on parties for select using(tenant_id=current_tenant_id());
...
create policy item_barcodes_read on item_barcodes for select using(tenant_id=current_tenant_id());
...
create policy item_prices_read on item_prices for select using(tenant_id=current_tenant_id());
...
create or replace function assert_phase3_shop(p_shop_id uuid) returns uuid language plpgsql stable security definer set search_path=public as $$
declare v_tenant uuid:=current_tenant_id(); begin
...
revoke all on function assert_phase3_shop(uuid) from public;

create or replace function post_purchase(p_shop_id uuid,p_party_id uuid,p_bill_no text,p_business_date date,p_discount_paise bigint,p_extra_charges_paise bigint,p_client_id text,p_lines jsonb,p_bill_image_path text default null)
returns uuid language plpgsql security definer set search_path=public as $$
declare
  v_tenant uuid; v_bill uuid; v_subtotal bigint:=0; v_total bigint; v_line jsonb; v_item items%rowtype;
  v_qty numeric; v_base_qty numeric; v_unit_level int; v_price bigint; v_line_total bigint; v_existing uuid;
begin
  if not has_perm('POST_PURCHASES') then raise exception 'not permitted'; end if;
  v_tenant:=assert_phase3_shop(p_shop_id);
  ...
  if p_discount_paise<0 or p_extra_charges_paise<0 then raise exception 'negative adjustment'; end if;
  if jsonb_typeof(p_lines)<>'array' or jsonb_array_length(p_lines)=0 then raise exception 'purchase requires lines'; end if;
  ...
  for v_line in select value from jsonb_array_elements(p_lines) loop
    ...
    v_unit_level:=coalesce((v_line->>'unit_level')::int,1); v_qty:=(v_line->>'qty')::numeric; v_price:=(v_line->>'unit_price_paise')::bigint;
    if v_qty<=0 or v_price<0 or v_unit_level not between 1 and 3 then raise exception 'invalid purchase line'; end if;
    if v_unit_level=1 then v_base_qty:=v_qty;
    elsif v_unit_level=2 and v_item.unit2 is not null and v_item.conv1>0 then v_base_qty:=v_qty*v_item.conv1;
    elsif v_unit_level=3 and v_item.unit3 is not null and v_item.conv1>0 and v_item.conv2>0 then v_base_qty:=v_qty*v_item.conv1*v_item.conv2;
  ...
  v_total:=v_subtotal-p_discount_paise+p_extra_charges_paise; if v_total<0 then raise exception 'negative total'; end if;
  ...
  values(v_tenant,p_shop_id,p_party_id,p_bill_no,coalesce(p_business_date,current_date),v_subtotal,p_discount_paise,p_extra_charges_paise,v_total,p_bill_image_path,p_client_id) returning id into v_bill;
  for v_line in select value from jsonb_array_elements(p_lines) loop
    select * into v_item from items where id=(v_line->>'item_id')::uuid and tenant_id=v_tenant;
    v_unit_level:=coalesce((v_line->>'unit_level')::int,1); v_qty:=(v_line->>'qty')::numeric; v_price:=(v_line->>'unit_price_paise')::bigint;
    if v_unit_level=1 then v_base_qty:=v_qty; elsif v_unit_level=2 then v_base_qty:=v_qty*v_item.conv1; else v_base_qty:=v_qty*v_item.conv1*v_item.conv2; end if;
    v_line_total:=round(v_qty*v_price)::bigint;
    insert into purchase_bill_items(tenant_id,shop_id,purchase_bill_id,item_id,item_name_at_purchase,unit_at_purchase,conv1_at_purchase,conv2_at_purchase,unit_level,qty,base_qty,unit_price_paise,line_total_paise,tax_rate_bp_at_purchase,hsn_at_purchase,client_id)
    values(v_tenant,p_shop_id,v_bill,v_item.id,v_item.name,case v_unit_level when 1 then v_item.unit1 when 2 then v_item.unit2 else v_item.unit3 end,v_item.conv1,v_item.conv2,v_unit_level,v_qty,v_base_qty,v_price,v_line_total,v_item.tax_rate_bp,v_item.hsn,p_client_id||':line:'||(v_line->>'item_id')||':'||v_unit_level::text);
    insert into stock_movements(tenant_id,shop_id,item_id,source_type,source_id,qty_base,client_id)
    values(v_tenant,p_shop_id,v_item.id,'PURCHASE',v_bill,v_base_qty,p_client_id||':stock:'||(v_line->>'item_id')||':'||v_unit_level::text);
  end loop;
  return v_bill;
exception when unique_violation then
  select id into v_existing from purchase_bills where tenant_id=v_tenant and client_id=p_client_id; if v_existing is not null then return v_existing; end if; raise;
end $$;

create or replace function void_purchase(p_purchase_id uuid,p_client_id text) returns uuid language plpgsql security definer set search_path=public as $$
declare v_bill purchase_bills%rowtype; v_line purchase_bill_items%rowtype; begin
  if not has_perm('POST_PURCHASES') then raise exception 'not permitted'; end if;
  ...
  for v_line in select * from purchase_bill_items where purchase_bill_id=v_bill.id and deleted_at is null loop
    insert into stock_movements(tenant_id,shop_id,item_id,source_type,source_id,qty_base,client_id)
    values(v_bill.tenant_id,v_bill.shop_id,v_line.item_id,'PURCHASE_VOID',v_bill.id,-v_line.base_qty,p_client_id||':stock:'||v_line.id::text);
  end loop;
  update purchase_bills set status='VOID',voided_at=now() where id=v_bill.id; return v_bill.id;
end $$;

create or replace function prevent_financial_mutation() returns trigger language plpgsql set search_path=public as $$ begin raise exception 'financial rows are immutable; use a void RPC'; end $$;
create trigger purchase_lines_immutable before update or delete on purchase_bill_items for each row execute function prevent_financial_mutation();
create trigger stock_movements_immutable before update or delete on stock_movements for each row execute function prevent_financial_mutation();
...
-- Soft-delete mutable master rows through one audited RPC. Referenced financial
-- history remains intact because the row itself is retained.
create or replace function archive_master(p_table text,p_id uuid) returns uuid language plpgsql security definer set search_path=public as $$
declare v_now bigint:=(extract(epoch from clock_timestamp())*1000)::bigint; begin
  ...
  if not found then raise exception 'row not found'; end if; return p_id;
end $$;
revoke all on function archive_master(text,uuid) from public; grant execute on function archive_master(text,uuid) to authenticated;
