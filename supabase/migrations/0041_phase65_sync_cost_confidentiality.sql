-- Phase 6.5 A1: preserve the item_prices RLS confidentiality rule through the
-- SECURITY DEFINER offline-sync feed. Applied migrations are forward-only, so
-- this replaces the live 0039 definition instead of editing migration history.
...
declare
  ...
  v_settings jsonb; v_allow_negative_stock boolean; v_can_view_cost_prices boolean;
...
begin
  ...
  v_can_view_cost_prices:=has_perm('POST_PURCHASES') or has_perm('VIEW_REPORTS');
  ...
  from (
    ...
    where p.tenant_id=v_tenant and p.updated_at<=v_cutoff
      and (v_can_view_cost_prices or p.kind<>'cost_last')
  ...
  return jsonb_build_object(
    ...
    'policy',jsonb_build_object('allowCashierOfflineFinalization',
      ...
      'canViewCostPrices',v_can_view_cost_prices),
