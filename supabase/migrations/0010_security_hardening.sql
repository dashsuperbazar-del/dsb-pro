...
exception
  when unique_violation then
    -- Idempotency is only valid for the caller's own (created_by, client_id)
    -- key. Do not swallow an unrelated unique violation such as a slug clash.
    select t.id into v_existing_tenant_id
    from tenants t
    where t.created_by = auth.uid() and t.client_id = p_client_id;
...
begin
  ...
  -- shop_ids is an array rather than a foreign-key column, so PostgreSQL cannot
  -- enforce tenant ownership automatically. Never allow a security-definer RPC
  -- to mint an invite carrying another tenant's shop IDs.
  if exists (
    select 1
