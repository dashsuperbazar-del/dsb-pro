...
select throws_ok(format($q$select record_customer_payment(%L::uuid,%L::uuid,'2026-09-08',1,'cash','over',jsonb_build_array(jsonb_build_object('sale_invoice_id',%L,'amount_paise',1)),'p4a-pay-over')$q$,current_setting('p4a.shop'),current_setting('p4a.customer'),current_setting('p4a.sale')),null,'allocation exceeds document outstanding amount','cumulative over-allocation is rejected');
select is((select count(*) from payments where client_id='p4a-pay-over'),0::bigint,'failed over-allocation rolls back its payment');
