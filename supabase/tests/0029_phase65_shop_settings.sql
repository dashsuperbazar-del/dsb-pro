...
select lives_ok($$select update_shop_settings(current_setting('st.shop')::uuid,'Renamed Shop','12 Market Road','27ABCDE1234F1Z5','RS','Asia/Kolkata','58mm',4::smallint)$$,'owner can update every shop setting');
...
select throws_ok($$select update_shop_settings(current_setting('st.shop')::uuid,'','','','','Asia/Kolkata','58mm',4::smallint)$$,null,'shop name required','blank name is rejected');
select throws_ok($$select update_shop_settings(current_setting('st.shop')::uuid,'Renamed Shop','','','','Asia/Kolkata','110mm',4::smallint)$$,null,'invalid printer width','unsupported printer width is rejected');
select throws_ok($$select update_shop_settings(current_setting('st.shop')::uuid,'Renamed Shop','','','','Asia/Kolkata','58mm',13::smallint)$$,null,'invalid fiscal year start month','out-of-range fiscal month is rejected');
...
select lives_ok($$select update_shop_settings(current_setting('st.shop')::uuid,'Manager Renamed','','','','Asia/Kolkata','80mm',1::smallint)$$,'manager can update shop settings');

select set_config('request.jwt.claims','{"sub":"b6520000-0000-0000-0000-000000000003","role":"authenticated"}',true);
select throws_ok($$select update_shop_settings(current_setting('st.shop')::uuid,'Cashier Renamed','','','','Asia/Kolkata','80mm',1::smallint)$$,null,'not permitted','cashier cannot change shop settings');
...
set role authenticated;
select set_config('request.jwt.claims','{"sub":"b6520000-0000-0000-0000-000000000001","role":"authenticated"}',true);
select throws_ok($$select update_shop_settings('b6520000-0000-0000-0000-000000000098'::uuid,'Cross Tenant','','','','Asia/Kolkata','80mm',1::smallint)$$,null,'shop not in tenant','owner of one tenant cannot change another tenant''s shop');
