...
select ok(not has_function_privilege('anon','update_shop_settings(uuid,text,text,text,text,text,text,smallint,boolean)','execute'),'anonymous cannot change shop settings');
...
select lives_ok($$select update_shop_settings(current_setting('st.shop')::uuid,'Renamed Shop','12 Market Road','27ABCDE1234F1Z5','RS','Asia/Kolkata','58mm',4::smallint,true)$$,'owner can update every shop setting');
...
select is((select allow_negative_stock from shops where id=current_setting('st.shop')::uuid),true,'allow_negative_stock persisted');

select throws_ok($$select update_shop_settings(current_setting('st.shop')::uuid,'','','','','Asia/Kolkata','58mm',4::smallint,true)$$,null,'shop name required','blank name is rejected');
select throws_ok($$select update_shop_settings(current_setting('st.shop')::uuid,'Renamed Shop','','','','Asia/Kolkata','110mm',4::smallint,true)$$,null,'invalid printer width','unsupported printer width is rejected');
select throws_ok($$select update_shop_settings(current_setting('st.shop')::uuid,'Renamed Shop','','','','Asia/Kolkata','58mm',13::smallint,true)$$,null,'invalid fiscal year start month','out-of-range fiscal month is rejected');
...
set role authenticated;
select set_config('request.jwt.claims','{"sub":"b6520000-0000-0000-0000-000000000001","role":"authenticated"}',true);
select throws_ok($$select update_shop_settings('b6520000-0000-0000-0000-000000000098'::uuid,'Cross Tenant','','','','Asia/Kolkata','80mm',1::smallint,false)$$,null,'shop not in tenant','owner of one tenant cannot change another tenant''s shop');
